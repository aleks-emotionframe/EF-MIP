    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-1 p-1 rounded bg-gray-100 dark:bg-white/[0.04] w-fit">
        {([
          ...
          <button
            ...
            className={`relative rounded px-5 py-2 text-[13px] font-medium transition-all ${
              mainTab === tab.key ? "text-gray-900 dark:text-white" : "text-gray-500 hover:text-gray-700 dark:hover:text-gray-300"
            }`}
          >
            {mainTab === tab.key && (
              <motion.div layoutId="seo-tab" className="absolute inset-0 bg-white dark:bg-[#1E293B] rounded shadow-sm" transition={{ type: "spring", stiffness: 400, damping: 30 }} />
            )}
      ...
      {mainTab === "analyse" && (
        <>
          ...
          <form onSubmit={handleAnalyze} className="flex gap-3">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full rounded border border-gray-200 dark:border-white/[0.06] bg-white dark:bg-[#1E293B] dark:text-white pl-11 pr-4 py-3.5 text-[14px] focus:border-[#00CEC9] focus:ring-2 focus:ring-[#00CEC9]/20 focus:outline-none transition-all"
              />
            </div>
          ...
          {error && <div className="rounded border border-red-200 bg-red-50 p-4 text-[13px] text-red-700">{error}</div>}
            ...
            {result && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                  <div className="rounded bg-white dark:bg-[#1E293B] shadow-sm p-6 flex flex-col items-center justify-center">
                  ...
                  <div className="rounded bg-white dark:bg-[#1E293B] shadow-sm p-5 flex flex-col justify-center space-y-3">
                  ...
                  <div className="lg:col-span-2 rounded bg-white dark:bg-[#1E293B] shadow-sm p-5">
                  ...
                  {categories.map((cat) => {
                    ...
                    return (
                      <button key={cat} onClick={() => setActiveCategory(cat)}
                        className={`flex items-center gap-1.5 rounded px-3 py-1.5 text-[12px] font-medium whitespace-nowrap transition-all ${activeCategory === cat ? "bg-[#00CEC9] text-white" : "bg-white dark:bg-[#1E293B] border border-gray-200 dark:border-white/[0.06] text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/[0.06]"}`}>
                        {cat === "all" ? "Alle" : cat}
                ...
                <div className="space-y-2">
                  {filteredChecks.map((check) => {
                    ...
                    return (
                      <div key={`${check.category}-${check.name}`} className="rounded bg-white dark:bg-[#1E293B] shadow-sm overflow-hidden">
                        <button onClick={() => setExpandedCheck(isExp ? null : `${check.category}-${check.name}`)} className="w-full flex items-center gap-3 p-4 text-left hover:bg-gray-50/50 dark:hover:bg-white/[0.06] transition-colors">
                          <div className={`w-8 h-8 rounded ${STATUS_BG[check.status]} flex items-center justify-center shrink-0`}><Icon className={`h-4 w-4 ${STATUS_COLOR[check.status]}`} /></div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2"><span className="text-[13px] font-semibold text-[#0F172A] dark:text-white">{check.name}</span><span className="text-[10px] text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-white/[0.04] rounded px-1.5 py-0.5">{check.category}</span></div>
                            <p className="text-[12px] text-gray-500 dark:text-gray-400 mt-0.5 truncate">{check.value}</p>
                          </div>
                        ...
                        <AnimatePresence>{isExp && (
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                            <div className="px-4 pb-4 pt-0"><div className={`rounded p-3.5 ${STATUS_BG[check.status]}`}><div className="flex items-start gap-2"><ArrowRight className={`h-3.5 w-3.5 mt-0.5 shrink-0 ${STATUS_COLOR[check.status]}`} /><p className="text-[13px] text-gray-700 leading-relaxed">{check.recommendation}</p></div></div></div>
                          </motion.div>
                        )}</AnimatePresence>
                      </div>
                    )
                    ...
                    <div className="space-y-1.5">
                      {result.headings.map((h, i) => (
                        <div key={i} className="flex items-center gap-2" style={{ paddingLeft: (parseInt(h.tag[1]) - 1) * 20 }}>
                          ...
                          <span className="text-[12px] text-gray-700 dark:text-gray-300 truncate">{h.text}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="rounded bg-white dark:bg-[#1E293B] shadow-sm p-5">
                    ...
                    <div className="space-y-1.5 max-h-48 overflow-y-auto">
                      {result.images.filter((i) => !i.hasAlt).map((img, i) => (
                        <div key={i} className="flex items-center gap-2 text-[12px] rounded p-2 bg-red-50">
        ...
        <div className="space-y-4">
            ...
            <div className="flex gap-1.5">
              {([
                ...
                <button key={s.key} onClick={() => setKeywordSort(s.key)}
                  className={`rounded px-3 py-2 text-[11px] font-medium transition-all ${keywordSort === s.key ? "bg-[#00CEC9] text-white" : "bg-white dark:bg-[#1E293B] border border-gray-200 dark:border-white/[0.06] text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/[0.06]"}`}>
                  {s.label}
                </button>
              ))}
            </div>
          ...
          <div className="space-y-2">
            {sortedKeywords.map((kw) => {
              ...
              return (
                <div key={kw.query} className="rounded bg-white dark:bg-[#1E293B] shadow-sm overflow-hidden">
                  <button onClick={() => setExpandedKeyword(isExp ? null : kw.query)}
                    ...
                    <div className="flex items-center gap-2 shrink-0 w-20">
                      <span className={`inline-flex items-center justify-center w-8 h-8 rounded text-[13px] font-bold ${posColor}`}>
                        {kw.position}
                      </span>
                    ...
                    {isExp && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                        <div className="px-4 pb-4 space-y-3">
                          ...
                          {/* Tip */}
                          <div className="rounded bg-[#00CEC9]/[0.04] border border-[#00CEC9]/10 p-4">
                            <div className="flex items-start gap-2.5">
                              <div className="w-7 h-7 rounded bg-[#00CEC9]/10 flex items-center justify-center shrink-0 mt-0.5">
                                <Lightbulb className="h-3.5 w-3.5 text-[#00CEC9]" />
                              </div>
                              <div>
                                <p className="text-[12px] font-semibold text-[#00CEC9] mb-1">Optimierungs-Tipp</p>
                                <p className="text-[13px] text-gray-700 dark:text-gray-300 leading-relaxed">{kw.tip}</p>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-3">
                            <span className="text-[11px] text-gray-400 dark:text-gray-500 w-24 shrink-0">Google-Position</span>
                            ...
                            <span className="text-[11px] font-medium text-gray-600 dark:text-gray-300 w-8 text-right">#{kw.position}</span>
                          </div>
                        </div>
                      </motion.div>
                    )}
