...
const stats = [
  { label: "Aktive Kampagnen", value: "12", change: "+3", trend: "up" as const, icon: Activity, gradient: "from-[#00CEC9] to-[#00A8A3]" },
  { label: "Engagement-Rate", value: "4.8%", change: "+0.6%", trend: "up" as const, icon: TrendingUp, gradient: "from-[#6C5CE7] to-[#5643CC]" },
  { label: "Kunden", value: "24", change: "+2", trend: "up" as const, icon: Users, gradient: "from-[#F97316] to-[#EA580C]" },
  { label: "KI-Erkenntnisse", value: "156", change: "-8", trend: "down" as const, icon: Sparkles, gradient: "from-[#E84393] to-[#D63384]" },
]

const activities = [
  { text: "Neue Emotion-Analyse für TechVision abgeschlossen", time: "Vor 12 Min.", dot: "#00CEC9" },
  { text: 'Kampagne "Sommer 2026" wurde aktiviert', time: "Vor 1 Std.", dot: "#6C5CE7" },
  { text: "3 neue Sentiment-Alerts für GreenLeaf", time: "Vor 2 Std.", dot: "#F97316" },
  { text: 'Report "Q1 Engagement" generiert', time: "Vor 4 Std.", dot: "#E84393" },
  { text: "KI-Erkenntnis: Positiver Trend bei MediaPulse erkannt", time: "Vor 5 Std.", dot: "#00CEC9" },
]
...
export default function DashboardPage() {
  return (
    <div className="space-y-6">
      {/* Greeting */}
      <div>
        <h1 className="text-2xl font-bold text-[#0F172A] dark:text-white">Willkommen zurück</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {stats.map((stat) => (
          <div key={stat.label} className="rounded-2xl bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
            ...
            <p className="text-[28px] font-extrabold text-[#0F172A] dark:text-white tracking-tight">{stat.value}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 rounded-2xl bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-[16px] font-bold text-[#0F172A] dark:text-white">Emotion Trends</h2>
              <p className="text-[12px] text-gray-500 dark:text-white/40 mt-0.5">Letzte 7 Tage</p>
            </div>
            <div className="flex items-center gap-5 text-[11px]">
              <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-[#00CEC9]" />Positiv</span>
              <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-[#F97316]" />Negativ</span>
          ...
          <div className="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <Area type="monotone" dataKey="positiv" stroke="#00CEC9" strokeWidth={2.5} fill="url(#gradPos)" />
                <Area type="monotone" dataKey="negativ" stroke="#F97316" strokeWidth={2} fill="url(#gradNeg)" />
        ...
        <div className="rounded-2xl bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-[16px] font-bold text-[#0F172A] dark:text-white">Aktivitäten</h2>
          ...
          <div className="space-y-5">
            {activities.map((activity, i) => (
              <div key={i} className="flex gap-3">
                ...
                <div className="pb-1">
                  <p className="text-[13px] text-[#0F172A] dark:text-white/80 leading-snug">{activity.text}</p>
      ...
      <div className="rounded-2xl bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-[16px] font-bold text-[#0F172A] dark:text-white">Top Kunden</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {topClients.map((client) => (
            <div key={client.name} className="flex items-center gap-3 rounded-xl bg-[#F4F7FE] dark:bg-white/[0.04] p-4 hover:bg-[#EEF2FF] dark:hover:bg-white/[0.06] transition-colors">
              <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-[#00CEC9]/20 to-[#6C5CE7]/10 flex items-center justify-center text-[13px] font-bold text-[#00CEC9] shrink-0">
                {client.name.split(" ").map(w => w[0]).join("").slice(0, 2)}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-semibold text-[#0F172A] dark:text-white truncate">{client.name}</p>
                <div className="flex items-center gap-2 mt-1">
                  <div className="flex-1 h-2 bg-white dark:bg-white/10 rounded-full overflow-hidden">
                    <div className="h-full rounded-full bg-gradient-to-r from-[#00CEC9] to-[#6C5CE7]" style={{ width: `${client.score}%` }} />
                  </div>
                  <span className="text-[12px] font-bold text-[#0F172A] dark:text-white">{client.score}</span>
