  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <button onClick={() => setShowAdd(true)} className="flex items-center gap-1.5 rounded bg-gradient-to-r from-[#00CEC9] to-[#6C5CE7] px-4 py-2.5 text-[12px] font-semibold text-white hover:shadow-lg hover:shadow-[#00CEC9]/30 transition-all">
        ...
        {competitors.map((c) => (
          <div key={c.id} className="relative group/comp">
            <button onClick={() => setSelectedId(c.id)}
              className={`rounded px-4 py-2.5 text-[12px] font-medium whitespace-nowrap transition-all ${selectedId === c.id ? "bg-[#00CEC9] text-white shadow-md shadow-[#00CEC9]/25" : "bg-white dark:bg-[#1E293B] border border-gray-200 dark:border-white/[0.06] text-gray-600 dark:text-white/70 hover:bg-gray-50 dark:hover:bg-white/[0.04]"}`}>
              {c.name}
            </button>
      ...
      {selected && (
        <div className="rounded bg-white dark:bg-[#1E293B] shadow-sm p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {selected.platforms.map((p) => {
              ...
              return (
                <div key={p.platform} className="rounded border border-gray-100 dark:border-white/[0.06] p-3.5">
                  ...
                  {yourPlatform && (
                    <div className={`mt-2 rounded px-2 py-1 text-[10px] font-medium text-center ${followerDiff > 0 ? "bg-red-50 text-red-600" : "bg-emerald-50 text-emerald-600"}`}>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <div className="space-y-3">
                <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Firmenname" className="w-full rounded border border-gray-200 px-4 py-2.5 text-[13px] focus:border-[#00CEC9] focus:outline-none" />
                <input value={newWebsite} onChange={(e) => setNewWebsite(e.target.value)} placeholder="Website (z.B. competitor.com)" className="w-full rounded border border-gray-200 px-4 py-2.5 text-[13px] focus:border-[#00CEC9] focus:outline-none" />
              </div>
              <div className="flex gap-3 mt-5">
                <button onClick={() => setShowAdd(false)} className="flex-1 rounded border border-gray-200 py-2.5 text-[13px] text-gray-600 hover:bg-gray-50">Abbrechen</button>
                <button onClick={handleAdd} disabled={!newName} className="flex-1 rounded bg-gradient-to-r from-[#00CEC9] to-[#6C5CE7] py-2.5 text-[13px] font-semibold text-white hover:bg-[#00B4A3] disabled:opacity-50">Hinzufügen</button>
              </div>
            </motion.div>
          </div>
        )}
